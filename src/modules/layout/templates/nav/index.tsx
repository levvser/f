...
const Nav: React.FC<NavProps> = ({ regions }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 48) {
        setNavClass("fixed top-0 transition-transform duration-300 ease-in-out backdrop-blur-3xl bg-white bg-opacity-80");
      } else {
        setNavClass("absolute top-12 transition-transform duration-300 ease-in-out bg-white bg-opacity-80");
      }
    };
  ...
  return (
    <header className={`w-full bg-opacity-80 z-50 ${navClass}`}>
